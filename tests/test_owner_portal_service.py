...
class TestOwnerPortalService(unittest.TestCase):
    ...
    def setUp(self):
        self.db = get_db()
        for table in ('invoice_requests', 'notification_events', 'payment_callbacks', 'payment_orders', 'owner_portal_sessions', 'owner_portal_login_codes', 'payments', 'invoices', 'bill_adjustments', 'bills', 'meter_readings', 'deposits', 'parking_spots', 'repairs', 'rooms', 'owners'):
