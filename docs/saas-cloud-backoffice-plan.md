...
- 平台系统健康摘要：首页对平台管理员展示部署资产、端口本机绑定、备份恢复脚本和 HTTPS 状态，并链接云端上线清单。
...
- 租户状态管理：平台管理员可在 `/backoffice/tenant-projects` 按客户名/状态筛选租户并停用/启用客户公司；停用租户员工登录和既有会话业务访问均返回 403，启用后恢复，状态变更写入目标租户审计。
