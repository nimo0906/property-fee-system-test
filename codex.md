...
#组件优化
- 后续涉及组件优化，优先查看https://ant.design/components/overview-cn/ 和21st.dev 组件网址查看相关适合的组件，以小样的形式呈现出来便于选择。
- 改动用户可见页面功能后，除普通 HTTP 页面验证外，还要考虑桌面启动器、打包资源、用户数据目录、备份目录和本地端口是否正常。

#UI设计
- 后续涉及UI页面优化，优先查看pageflows.com ， godly.website  ， mobbin.com等网站查看相关适合的UI页面设计，以小样的形式呈现出来便于选择。
- 改动用户可见页面功能后，除普通 HTTP 页面验证外，还要考虑桌面启动器、打包资源、用户数据目录、备份目录和本地端口是否正常。
