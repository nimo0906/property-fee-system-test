...
def _billing_check_panel():
    return '''<section class="card" style="margin-bottom:18px"><div class="card-h">出账检查</div><div class="card-b"><div class="actions"><span class="badge">账期</span><span class="badge">服务期起止</span><span class="badge">金额核对</span><span class="badge">重复出账自动跳过</span><a class="ghost-link" href="/backoffice/fee-types">检查收费项目</a><a class="ghost-link" href="/backoffice/charge-targets">检查收费对象</a><a class="ghost-link" href="/backoffice/payments">收款登记</a></div><div class="hint">建议先预览批量出账，确认数量、金额、账期和服务期无误后再写入账单；审核后进入正式应收和收款。</div></div></section><section class="card" style="margin-bottom:18px"><div class="card-h">账单查询流程</div><div class="card-b"><div class="work-grid"><a class="work-card primary-work-card" href="/backoffice/bills"><strong>生成/预览账单</strong><span>单户生成或批量出账预览</span></a><a class="work-card" href="/backoffice/bills?status=pending_review"><strong>审核应收账单</strong><span>待审核账单审核后进入应收</span></a><a class="work-card" href="/backoffice/bills?status=unpaid"><strong>查询未收欠费</strong><span>筛选未收款账单和欠费金额</span></a><a class="work-card" href="/backoffice/payments"><strong>进入收款登记</strong><span>按账单登记本次收款</span></a><a class="work-card" href="/backoffice/reports"><strong>核对欠费报表</strong><span>按楼栋、项目和收费项核对</span></a></div></div></section>'''
...
def _batch_generate_form(fees):
    ...
    fee_options = _options(fees, lambda item: f"{item.get('name')} · {item.get('unit_price')}")
    category_options = '<option value="">全部类型</option><option value="居民">仅居民</option><option value="商户">仅商户</option><option value="商业">商业/商户</option><option value="办公">仅办公</option><option value="其他">仅其他</option>'
