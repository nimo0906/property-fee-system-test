...
class ReportMixinPart3(BaseHandler):
    ...
    def _reports(self, q):
        ...
        <div class="report-section-grid"><div class="card report-panel report-compact-table"><div class="card-header d-flex justify-content-between align-items-center gap-2"><span>主要收费类型统计</span><small class="text-muted fw-normal">低频有金额项目合并到其他收费</small></div>{fee_table}</div><div class="card report-panel report-compact-table"><div class="card-header">收款日报</div>{daily_table}</div></div>
        <div class="report-section-grid"><div class="card report-panel report-compact-table"><div class="card-header">楼栋缴费统计</div>{bld_table}</div><div class="card report-panel report-compact-table"><div class="card-header"><i class="bi bi-exclamation-triangle text-danger"></i> 欠费排行Top10</div>{top_table}</div></div>
