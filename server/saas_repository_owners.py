...
def attach_owner_repository_methods(cls):
    ...
    def create_charge_target(self, tenant_id, project_id, building, unit, room_number, category, area, owner_id=0):
        self._require_project_scope(tenant_id, project_id)
        ...
        with self.engine.begin() as conn:
            result = conn.execute(text("""INSERT INTO charge_targets(tenant_id,project_id,owner_id,building,unit,room_number,category,area)
                VALUES(:tenant_id,:project_id,:owner_id,:building,:unit,:room_number,:category,:area)"""),
                {"tenant_id": tenant_id, "project_id": project_id, "owner_id": owner_id or None, "building": building, "unit": unit, "room_number": room_number, "category": category, "area": float(area)})
            return {"id": result.lastrowid, "tenant_id": tenant_id, "project_id": project_id, "owner_id": owner_id or None, "owner_name": owner.get("name") if owner else "", "owner_phone": owner.get("phone") if owner else "", "building": building, "unit": unit, "room_number": room_number, "category": category, "area": float(area)}

    def get_charge_target(self, tenant_id, project_id, target_id):
        return self._row("""SELECT ct.id,ct.tenant_id,ct.project_id,ct.owner_id,o.name owner_name,o.phone owner_phone,ct.building,ct.unit,ct.room_number,ct.category,ct.area FROM charge_targets ct LEFT JOIN owners o ON ct.owner_id=o.id AND ct.tenant_id=o.tenant_id AND ct.project_id=o.project_id
    ...
    def list_charge_targets(self, tenant_id, project_id):
        with self.engine.begin() as conn:
            rows = conn.execute(text("""SELECT ct.id,ct.tenant_id,ct.project_id,ct.owner_id,o.name owner_name,o.phone owner_phone,ct.building,ct.unit,ct.room_number,ct.category,ct.area FROM charge_targets ct LEFT JOIN owners o ON ct.owner_id=o.id AND ct.tenant_id=o.tenant_id AND ct.project_id=o.project_id
