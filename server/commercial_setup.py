...
class CommercialSetupMixin(BaseHandler):
    def _license_status(self):
        ...
        <div class="alert alert-light border mt-3 small"><i class="bi bi-info-circle"></i> 后续接正式商业授权时，可由授权服务器签发 license.json；本机只保存授权状态，不保存云端密钥，不在日志输出敏感授权明文。</div>
        """, "license_status"))
    ...
    def _first_run_guide(self):
        ...
        <div class="card"><div class="card-header">正式使用前 8 步</div><div class="card-body"><ol class="mb-0">
          ...
          <li>按“房间/商户 → 抄表 → 出账 → 收费登记 → 收据/报表”的顺序跑一遍小样本。</li>
