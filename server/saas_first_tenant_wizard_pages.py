...
def _delivery_state(message='', tenant_name='', project_name='', license_customer_code='', business_template_code='residential'):
...
<section class="grid"><div class="card"><div class="card-h">首租户资料</div><div class="card-b"><form method="post" action="/backoffice/first-tenant-wizard/create"><label>客户公司 / 租户名称</label><input name="tenant_name" required placeholder="例如 金桥物业"><label>默认项目名称</label><input name="project_name" required placeholder="例如 金桥一期"><label>项目编码</label><input name="project_code" placeholder="例如 JQ-001"><label>租户管理员账号</label><input name="admin_username" required placeholder="例如 tenant_admin_jq"><label>临时密码</label><input type="password" name="admin_password" required minlength="8"><label>授权客户编号</label><input name="license_customer_code" required placeholder="例如 CUST-JQ-001"><label>业务模板</label><select name="business_template">{template_options(business_template_code)}</select><button class="primary">创建首租户并绑定授权</button><div class="hint">临时密码只用于初始化，不在页面、日志或审计明细展示。授权绑定只写系统侧映射，不写客户业务表。</div></form></div></div>
<aside class="card"><div class="card-h">隔离边界</div><div class="card-b"><p class="sub">客户上传数据与系统自身数据隔离。</p><p class="sub">业务数据不进入授权云服务。</p><p class="sub">收费对象、账单、收款、导入和报表都归属新客户公司和项目。</p><p class="sub">授权绑定写入系统侧授权映射，并保留审计。</p></div></aside></section>{render_template_summary(business_template_code)}
...
def register_first_tenant_wizard_pages(app, service, repository, current_user):
    ...
    def wizard_page(user=Depends(current_user), message: str = '', tenant_name: str = '', project_name: str = '', license_customer_code: str = '', business_template: str = 'residential'):
        try:
            _require_platform(user)
            return HTMLResponse(_delivery_state(message, tenant_name, project_name, license_customer_code, business_template))
        except PermissionDenied:
            raise HTTPException(status_code=403, detail='forbidden')

    @app.post('/backoffice/first-tenant-wizard/create')
    def wizard_create(tenant_name: str = Form(...), project_name: str = Form(...), admin_username: str = Form(...), admin_password: str = Form(...), license_customer_code: str = Form(...), business_template: str = Form('residential'), project_code: str = Form(''), user=Depends(current_user)):
        try:
            _require_platform(user)
