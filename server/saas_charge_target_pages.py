...
def _render_targets(user, items, message='', filters=None, page=1, page_size=20, total=0, owners=None):
    filters = filters or {}
    owners = owners or []
    rows = ''.join(_row(item) for item in items) or '<tr><td colspan="8">暂无收费对象</td></tr>'
...
<section class="grid"><div class="card"><div class="card-h">收费对象列表</div><div class="card-b">{pager}<table><thead><tr><th>ID</th><th>楼栋 / 区域</th><th>单元 / 分区</th><th>房号 / 铺位号</th><th>业主</th><th>联系电话</th><th>类型</th><th>面积</th></tr></thead><tbody>{rows}</tbody></table>{pager}</div></div>
<aside class="card"><div class="card-h">新增收费对象</div><div class="card-b">{form}</div></aside></section>'''
    return _page('收费对象管理', body)


def _row(item):
    return f'''<tr><td>{_h(item.get('id'))}</td><td>{_h(item.get('building'))}</td><td>{_h(item.get('unit'))}</td><td><strong>{_h(item.get('room_number'))}</strong></td><td>{_h(item.get('owner_name') or '未绑定')}</td><td>{_h(item.get('owner_phone') or '')}</td><td>{_h(item.get('category'))}</td><td>{_h(item.get('area'))}</td></tr>'''
...
def register_charge_target_pages(app, service, repository, current_user):
    ...
    def create_charge_target_page(building: str = Form(...), unit: str = Form(''), room_number: str = Form(...), category: str = Form('居民'), area: float = Form(...), owner_id: int = Form(0), user=Depends(current_user)):
        try:
            service._require(user, 'write')
            if repository:
                item = repository.create_charge_target(user['tenant_id'], user['project_id'], building, unit, room_number, category, area, owner_id)
                service._log(user, user['project_id'], 'charge_target.create', 'charge_target', item['id'], {'building': building, 'room_number': room_number})
            else:
                service.create_charge_target(user, user['project_id'], building, unit, room_number, category, area, owner_id)
