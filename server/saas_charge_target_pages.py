...
def _render_targets(user, items, message='', filters=None, page=1, page_size=20, total=0):
    filters = filters or {}
...
<section class="grid"><div class="card"><div class="card-h">收费对象列表</div><div class="card-b">{pager}<table><thead><tr><th>ID</th><th>楼栋 / 区域</th><th>单元 / 分区</th><th>房号 / 铺位号</th><th>类型</th><th>面积</th></tr></thead><tbody>{rows}</tbody></table>{pager}</div></div>
...
def register_charge_target_pages(app, service, repository, current_user):
    ...
    def charge_target_page(user=Depends(current_user), message: str = '', building: str = '', unit: str = '', room_number: str = '', category: str = '', page: int = 1, page_size: int = 20):
        try:
            ...
            return HTMLResponse(_render_targets(user, visible, message, filters, page, page_size, total))
