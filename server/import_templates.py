...
TEMPLATES = {
    'basic': {
        ...
        'headers': ['项目','楼栋/区域','单元/分区','房号/铺位号','楼层','对象类型','面积㎡','物业费单价','水费标准','业主姓名','业主电话','身份证号','租户姓名','租户电话','租户身份证号','合同开始日期','合同结束日期','缴费周期','店铺名称','业态/商户类别','备注'],
        'rows': [['默认项目','商业区','一层','SP-101',1,'商户',88.5,4.8,'非居民','甲商贸','13900000000','','李四','13800000001','','2026-01-01','2026-12-31','季付','某某便利店','餐饮','基础资料模板，不填写历史收款金额'], ['默认项目','住宅楼','1单元','902',9,'居民',95.2,'','非居民','王五','13800000000','','','','','2026-01-01','2026-12-31','月付','','','住宅示例，仅填写基础资料']],
        'notes': [['用途','导入收费对象、业主、租户、面积、合同期等基础资料；层级为项目 → 楼栋/区域 → 单元/分区 → 房号/铺位号。'], ['提醒','历史收款金额不要填在本模板，收款明细请用收款明细识别模板。']],
    },
    'owners': {
    ...
    'payment_ledger': {
        ...
        'rows': [['2026-05-31','住宅楼902','王五',500,500,300,100,80,20,0]],
        'notes': [['用途','只做收款明细识别和收费项目映射，不自动入账。'], ['必填','房间号、用户名称、本期金额、本期收款(合计)建议保留。']],
    },
    'bills': {
        'sheet': '账单记录模板', 'filename': 'bills_template.xlsx',
        'headers': ['楼栋/区域','房号/铺位号','费用项目','账期','金额','状态','已缴','缴费日期','缴费方式','备注'],
        'rows': [['住宅楼','902','物业费(居民)','2026-05',180.88,'unpaid','','','','谨慎使用，建议先备份并预览']],
        'notes': [['用途','导入历史账单记录。'], ['提醒','账单导入会影响财务核对，正式使用前必须预览核对。']],
    },
    'b_tower_contracts': {
        'sheet': '出租合同模板', 'filename': 'b_tower_contracts_template.xlsx',
        'headers': ['楼栋/区域','单元/分区','房号/铺位号','租户','租户电话','面积','物业费单价','缴费周期','合同开始日期','合同结束日期','水费标准','备注'],
        'rows': [['住宅楼','1单元','902','李四','13800000001',95.2,1.9,'月付','2026-01-01','2026-12-31','非居民','默认只更新已有收费对象']],
        'notes': [['用途','写入收费对象管理的租户、合同期、单价、周期等字段。'], ['提醒','默认不新建缺失房间，预览页可勾选允许新建。']],
    },
