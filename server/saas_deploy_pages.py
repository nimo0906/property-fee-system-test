...
<section class="card" style="margin-top:18px"><div class="card-h">上线步骤</div><div class="card-b"><ol><li>准备 .env.example 对应的生产环境变量，不提交真实 .env。</li><li>执行 scripts/saas_preflight_check.py。</li><li>执行 scripts/saas_acceptance_check.py。</li><li>使用 docker compose up -d 启动 PostgreSQL 和 app。</li><li>确认应用仅绑定 127.0.0.1:8000。</li><li>配置 deploy/nginx/property-saas.conf 进行 HTTPS 终止。</li><li>执行 scripts/saas_backup.sh 和 scripts/saas_restore.sh --verify-metadata 做演练。</li></ol><div class="hint">本页只展示 checklist，不展示 POSTGRES_PASSWORD、APP_SECRET_KEY 或其他敏感值。</div></div></section>'''
