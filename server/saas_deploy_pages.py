...
<section class="card" style="margin-top:18px"><div class="card-h">生产部署一键自检</div><div class="card-b"><ul><li>命令：scripts/saas_production_precheck.py。</li><li>说明：docs/saas-production-precheck.md。</li><li>覆盖服务器环境、端口与反向代理、Docker Compose、Nginx HTTPS、systemd、logrotate。</li><li>检查客户上传数据目录、系统自身数据目录、备份目录、日志目录必须分离。</li><li>检查备份恢复脚本、授权绑定文件、上线证据文件。</li><li>失败即停止交付，修复后重新执行 scripts/saas_release_gate.py。</li></ul></div></section>
...
<section class="card" style="margin-top:18px"><div class="card-h">首租户业务冒烟</div><div class="card-b"><ul><li>检查：scripts/saas_production_first_tenant_smoke.py。</li><li>本地 CI：PYTHONPYCACHEPREFIX=/tmp/property_pycache python3 scripts/saas_production_first_tenant_smoke.py --local-testclient。</li><li>生产执行：PYTHONPYCACHEPREFIX=/tmp/property_pycache python3 scripts/saas_production_first_tenant_smoke.py --base-url https://your-domain.example.com。</li><li>覆盖登录、收费对象、收费项目、出账、收款、报表、导出、租户隔离。</li><li>只使用冒烟测试租户和测试账期，不导入真实客户数据。</li></ul></div></section>
<section class="card" style="margin-top:18px"><div class="card-h">生产运行状态自检</div><div class="card-b"><ul><li>检查：scripts/saas_production_runtime_check.py。</li><li>dry-run：PYTHONPYCACHEPREFIX=/tmp/property_pycache python3 scripts/saas_production_runtime_check.py --dry-run。</li><li>服务器执行：PYTHONPYCACHEPREFIX=/tmp/property_pycache python3 scripts/saas_production_runtime_check.py。</li><li>覆盖 docker compose ps、systemctl is-active property-saas、curl -fsS http://127.0.0.1:8000/health。</li><li>覆盖 curl -fsS https://your-domain.example.com/login、nginx -t、日志目录可写。</li><li>确认 app 只监听本机端口，不允许 0.0.0.0:8000 直接暴露。</li></ul></div></section>
<section class="card" style="margin-top:18px"><div class="card-h">生产 .env 现场校验</div><div class="card-b"><ul><li>检查：scripts/saas_production_env_file_check.py。</li><li>服务器执行：PYTHONPYCACHEPREFIX=/tmp/property_pycache python3 scripts/saas_production_env_file_check.py --env-file .env。</li><li>权限要求：chmod 600 .env。</li><li>检查真实 .env 是否存在、权限是否过宽、必填项是否齐全、密钥是否仍是占位值。</li><li>检查客户上传数据目录、系统自身数据目录、备份目录和日志目录保持隔离。</li><li>不打印密钥原文，只输出变量名和长度。</li></ul></div></section>
<section class="card" style="margin-top:18px"><div class="card-h">生产部署实施命令</div><div class="card-b"><ul><li>文档：docs/saas-production-deployment-commands.md。</li><li>检查：scripts/saas_production_deployment_commands_check.py。</li><li>创建环境文件：cp .env.example .env，并执行 chmod 600 .env。</li><li>容器启动：docker compose pull；docker compose up -d。</li><li>systemd：sudo install -m 0644 deploy/systemd/property-saas.service /etc/systemd/system/property-saas.service；sudo systemctl daemon-reload；sudo systemctl enable --now property-saas。</li><li>服务状态：sudo systemctl status property-saas --no-pager。</li><li>健康检查：curl -fsS http://127.0.0.1:8000/health。</li><li>登录检查：curl -fsS https://your-domain.example.com/login。</li><li>上线门禁：PYTHONPYCACHEPREFIX=/tmp/property_pycache python3 scripts/saas_release_gate.py。</li><li>失败即停止交付，修复后重新执行对应检查。</li></ul></div></section>
<section class="card" style="margin-top:18px"><div class="card-h">上线步骤</div><div class="card-b"><ol><li>准备 .env.example 对应的生产环境变量，不提交真实 .env。</li><li>执行 scripts/saas_env_security_check.py，完成生产环境变量安全检查。</li><li>执行 scripts/saas_preflight_check.py。</li><li>执行 scripts/saas_acceptance_check.py。</li><li>执行 scripts/saas_ops_check.py。</li><li>执行 scripts/saas_phase1_closure_check.py，确认第一阶段收口。</li><li>执行 scripts/saas_demo_tenant_drill.py，完成样例客户验收演练。</li><li>执行 scripts/saas_deployment_drill_check.py，确认云端部署演练资产完整。</li><li>执行 scripts/saas_production_deployment_rehearsal_check.py，确认实机部署演练清单完整。</li><li>执行 scripts/saas_production_precheck.py，完成生产部署一键自检。</li><li>执行 scripts/saas_minimal_launch_package_check.py，确认最小上线包完整。</li><li>执行 scripts/saas_release_gate.py，作为商业上线总门禁。</li><li>执行 scripts/saas_isolation_evidence.py，生成租户隔离证据明细。</li><li>执行 scripts/saas_release_evidence.py，生成上线证据报告。</li><li>使用 docker compose up -d 启动 PostgreSQL 和 app。</li><li>确认应用仅绑定 127.0.0.1:8000。</li><li>配置 deploy/nginx/property-saas.conf 进行 HTTPS 终止。</li><li>安装 deploy/logrotate/property-saas：日志轮转安装。</li><li>按管理员重置密码流程抽查租户管理员和平台管理员边界。</li><li>执行 scripts/saas_backup.sh 和 scripts/saas_restore.sh --verify-metadata 做演练，并保留恢复演练证据。</li></ol><div class="hint">本页只展示 checklist，不展示敏感字段名、内部租户字段或真实密钥值。</div></div></section>'''
