...
<section class="grid"><div class="card"><div class="card-h">部署资产</div><div class="card-b"><ul><li>scripts/saas_preflight_check.py</li><li>scripts/saas_acceptance_check.py</li><li>scripts/saas_ops_check.py</li><li>docs/saas-cloud-ops-runbook.md</li><li>docker compose up -d</li><li>deploy/nginx/property-saas.conf</li><li>deploy/systemd/property-saas.service</li><li>scripts/saas_backup.sh</li><li>scripts/saas_restore.sh --verify-metadata</li></ul><div class="hint">已存在文件：<ul>{file_items}</ul></div><div class="hint">缺失文件：<ul>{missing_items}</ul></div></div></div>
