...
<aside class="card"><div class="card-h">客户数据隔离</div><div class="card-b"><p class="sub">租户管理员只能管理本公司员工账号，不能查看或操作其他客户公司账号；所有账号操作写入本租户审计日志，不记录密码明文。</p><p class="sub">客户上传数据只进入本租户目录，系统自身部署、备份和运行数据使用系统目录，不能与客户业务数据混放。</p><a class="ghost-link" href="/backoffice/audit-logs">查看审计日志</a></div></aside></section>'''
