...
from server.db import get_db, is_period_closed, h, m, qs
...
class ClosingMixin(BaseHandler):
    ...
    def _closing(self):
        ...
        closed_periods=set(r['period'] for r in records if r['status']=='closed')
        ...
        for r in records:
            ...
            rhtml+='<tr><td>'+h(r['period'])+'</td>'
            st='<span class="badge bg-success">已结账</span>' if r['status']=='closed' else '<span class="badge bg-warning text-dark">已反结账</span>'
            rhtml+='<td>'+st+'</td>'
            rhtml+='<td>'+(r['close_date'] or '-')+'</td>'
            rhtml+='<td>'+h(r['operator'] or '-')+'</td>'
            rhtml+='<td>'+str(r['paid_count'] or 0)+'笔</td>'
            rhtml+='<td class="text-end">¥'+m(r['total_amount'] or 0)+'</td>'
            rhtml+='<td>'+redos+'</td></tr>'
        popts=''
        for p in periods:
            pp=p['billing_period']
    ...
    def _closing_close(self, d):
        period=qs(d,'period')
        if not period: return self._redirect('/closing?flash=请选择账期')
        if is_period_closed(period): return self._redirect('/closing?flash='+period+'已结账')
        db=get_db()
        stats=self._closing_period_stats(db, period)
        if qs(d,'confirm')!='1':
            db.close()
            return self._render_closing_precheck(period, qs(d,'notes'), stats)
        paid=db.execute("SELECT COUNT(*) cnt, COALESCE(SUM(amount),0) tot FROM bills WHERE billing_period=? AND status='paid'",(period,)).fetchone()
    ...
    def _closing_period_stats(self, db, period):
        row=db.execute("""
    ...
    def _render_closing_precheck(self, period, notes, stats):
        warnings=[]
        ...
        self._html(self._page('结账前检查',f'''
        <div class="alert alert-warning"><strong>结账前检查</strong>：请先核对 {h(period)} 的账单状态和金额，确认后才会正式结账。</div>
        ...
        <form method=POST action="/closing/close">
    ...
    def _closing_reopen(self, d):
        period=qs(d,'period')
        if not period: return self._redirect('/closing?flash=参数错误')
        if qs(d,'confirm')!='1':
    ...
    def _render_reopen_confirm(self, period):
        ...
        <div class="alert alert-danger"><strong>反结账确认</strong>：请确认要重新打开账期 <code>{h(period)}</code>。</div>
        <div class="card"><div class="card-header">原结账信息</div><div class="card-body"><table class="table table-borderless mb-0">
            <tr><td class="text-muted" style="width:130px">账期</td><td>{h(period)}</td></tr>
        ...
        <div class="alert alert-warning"><strong>风险提示</strong>：反结账后该账期将重新允许生成账单、收费、修改、删除和批量操作，请确认这是有意操作。</div>
        <form method=POST action="/closing/reopen" onsubmit="return confirm('确认反结账？该账期将重新开放修改。')">
