...
from desktop_runtime import (
    APP_DIR_NAME, APP_TITLE, build_window_model, find_free_port,
...
def run_gui():
    ...
    tk.Button(buttons, text="打开数据目录", width=14, command=lambda: open_folder(data_dir)).grid(row=0, column=1, padx=6, pady=4)
    tk.Button(buttons, text="查看诊断信息", width=14, command=show_diagnostics).grid(row=1, column=0, padx=6, pady=4)
    tk.Button(buttons, text="打开错误日志", width=14, command=open_startup_log).grid(row=1, column=1, padx=6, pady=4)
...
def main():
    ...
    try:
        if args.diagnose:
            ...
            return
        if args.serve_only:
            run_server(args.port or find_free_port())
        else:
            run_gui()
